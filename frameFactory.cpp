...
FrameFactory::~FrameFactory() {
  ...
  std::map<std::string, Frame*>::const_iterator ptr2 = frames.begin();
  while ( ptr2 != frames.end() ) {
    delete ((ptr2->second));
    ++ptr2;
  }
  
  std::map<std::string, std::vector<SDL_Surface*> >::const_iterator ptr3 = multiSurfaces.begin();
  while ( ptr3 != multiSurfaces.end() ) {
    for (unsigned int i = 0; i < ptr3->second.size(); ++i) {
      SDL_FreeSurface( ptr3->second[i] );
...
Frame* FrameFactory::getFrameFromSurface(const std::string& name, SDL_Surface* surface) {
